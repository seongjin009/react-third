//비동기 데이터 frtching함수 등록
//비동기 데이터의 상태에 따라서 자동으로 해당 데이터를 변경할 수 있는 리듀서라는 함수를 등록

import axios from 'axios';
import { createAsyncThunk, createSlice } from '@reduxjs/toolkit';

//비동기 서버통신으로 데이터를 전달받아서 내부적으로 action차입을 자동으로 생성해서 액션개체 생성까지 완료하는 함수
...
//createAsyncThunk가 반환하는 acrion객체를 받아서 전역스토어 데이터를 변현하는 reducer 함수등록
...
/*
  ...
  dispatch : reducer에 데이터 변경 요청을 해주는 함수 (전달자) (창구직원)
  action : dispatch로 리듀서에 데이터 변경요청을 위해 필요한 특별한 형태의 객체 (창구직원에게 전달하기 위한 지로용지)
  action객체의 구조 : {type, payload}

  1 컴포넌트에서 데이터변경이나 데이터 요청을 위한 action객체를 만들어서 dispatch함수에 전달
  2 dispatch는 action객체를 가지고 리듀서 함수에 전달
  3 reducer함수는 dispatch가 전달되는 action객체의 타입에 따라 store의 데이터 변경 처리
  4 변경 store데이터는 index.js에 의해서 app.jsx에 전달됨
  5 루트 컴포넌트인 app.jsx에 데이터가 전달되었기 떄문에 자식 컴포넌트 어디에서는 ㄴ해당 데이터에 접근 가능
*/
